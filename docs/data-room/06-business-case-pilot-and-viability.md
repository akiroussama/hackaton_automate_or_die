...
The learned recommender is intended to reduce navigation effort by suggesting
one of the three already-feasible routes. It does not create the plans,
establish plant value or remove the production manager from the decision.
...
| 5. Assess learned assist | Confirm whether enough representative history exists; if so, retrain and validate on-site | Versioned model card, holdout result and rollback gate |
| 6. Shadow | Run beside the current process on qualifying incidents; manager remains in control | Decision-time, feasibility and usability log |
| 7. Decide | Review pre-agreed gates with the sponsor | Go, revise or stop decision |
...
Five to ten replays are a workflow and constraint-validation starting point,
not automatically a sufficient machine-learning dataset. The current model was
trained only on **687 simulated incidents generated by the twin itself**. Any
pilot retraining uses sufficient real incident history inside the site's
approved environment; data never leaves the plant, and a separate
representative holdout must pass before the learned assist is enabled.
...
- an on-site model owner and version/rollback process if retraining is in scope;
...
| High-fidelity digital twin | Rich physical or process representation | Adds an isometric decision view without requiring CAD, physics or full plant digitization |
...
- a transparent, on-site retraining and model-validation process;
- integration into the actual approval workflow.

That asset can improve through controlled, versioned retraining once enough
representative incidents exist. The current prototype does not learn online.
