...
- preview a revised Gantt and the same schedule in an interactive isometric
  workshop twin before action;
- use a separate local learned model to suggest one of the three already-built
  routes, with visible confidence and explanatory factors;
...
industrial performance. The recommender was trained on **687 simulated
incidents generated by the twin itself**; its 93.6% held-out test accuracy is a
synthetic-grid result, not evidence of real-plant accuracy.
...
- an interactive isometric decision view with clearly labelled simulated
  telemetry, operators and ambient signals;
- a local recommender that suggests, but never creates or alters, a route;
- a public static demonstration and an offline-capable local build;
...
- providing a CAD-, physics- or sensor-connected replica of a real factory;
...
| Learned-assist fact | A local model classifies held-out synthetic scenarios at 93.6% accuracy | Generalization, calibration and usefulness on real plant incidents |
...
autonomous deployment. Early replay validates the workflow and constraint
model; it does not by itself provide enough evidence to trust a production
recommender. Once sufficient representative history exists, retraining and
validation would run on-site so the plant's data never leaves its environment.
