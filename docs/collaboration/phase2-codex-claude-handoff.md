...
**Current owner:** Codex
**Status:** ACTIVE — documentation-update block on direct team-leader order
(~16:50 local: "on passe la main a GPT pour faire tout le reste ... droit
vers les livrables avec les nouvelles améliorations"). Iteration 3 (factory
twin + ML recommender) is DELIVERED, merged and LIVE: `main` = `origin/main`
= `17535cd`, which includes the fast-forwarded factory-twin commits AND the
cherry-picked Codex diagram `28a184f` (PNG/PDF byte-match Codex's reported
hashes; SVG differs only by CRLF checkout normalization). See the complete
2026-07-18 17:00 Claude Code handoff entry below: repository state, full
feature inventory, verification evidence, RED LINES for every document
update, suggested scope, and the Option A / Option B decision with the
18:45 fallback rule.
**Task:** Update the documents per the 17:00 handoff (README, data-room
sections, cabletwin docs; deck/data-room PDF regeneration only under the
Option B conditions). Respect every red line — especially the ML wording
("687 simulated incidents generated by the twin itself"; pilot = on-site
retraining), the unchanged canonical facts, the separate "9/9" vs
"check:recommender 5/5" claims, and the untouchable tags/`packaging/`.
The frozen 1:57.9 MP4 remains tonight's submitted video in every scenario.
**Timeline priority:** the team leader's SwissTransfer uploads + form
submission (accepted runbook, ceiling 19:30 local, still UNCONFIRMED at
17:00) outrank all documentation work.
**Parked with Claude Code:** the bounded Iteration 2 correction (14:59
spec). Claude stands by for the README block or Iteration 2 on lock return
and answers technical questions about the twin/recommender on demand.
